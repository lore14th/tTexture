...
namespace tTexture {
		...
		: m_Filepath(filepath), m_DesiredChannels(desiredChannels), m_FlipOnLoad(flipOnLoad)
	{
		...
		if (m_Filepath == std::string())
		{
			...
			CORE_FATAL("TextureLoader: You must provide a valid filepath");
			exit(-1);
		}
		// Assert that desired channels is 3 or 4
		if (m_DesiredChannels < 3 || m_DesiredChannels > 4)
		{
			std::cout << "ERROR! TextureLoader: desiredChannels must be 3 or 4. desired channels = " << m_DesiredChannels << std::endl;
			CORE_FATAL("TextureLoader: desiredChannels must be 3 or 4. desiredChannels = {0}", m_DesiredChannels);
			exit(-2);
		}
	}
	...
	void Loader::LoadImageFromFile(Texture2D& result)
	{
		...
		CORE_INFO("TextureLoader: Loading Texture {0}, Flip on Load = {1}", m_Filepath, m_FlipOnLoad);
