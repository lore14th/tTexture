...
namespace tTexture::Renderer {
	...
	OpenGLTextureCube::OpenGLTextureCube(const TextureCube& texture)
	...
	void OpenGLTextureCube::CreateTextureCommand()
	{
		...
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[2].Data);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[0].Data);

		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[4].Data);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[5].Data);

		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[1].Data);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, internalFormat, m_FaceSize, m_FaceSize, 0, pixelFormat, GL_UNSIGNED_BYTE, m_Texture.Images[3].Data);
	...
	tTexture::byte* OpenGLTextureCube::GetPixels(uint32_t face, uint32_t& size) const
	{
		size = m_FaceSize * m_FaceSize * m_Texture.Data.Bpp;
		byte* pixels = new byte[size];

		glGetTexImage(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, 0, m_Texture.Data.Bpp == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, pixels);
